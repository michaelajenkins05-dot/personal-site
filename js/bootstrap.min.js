...
// Entry Array to store journal entries
let entries = [];
...
function addEntry() {
    ...
    if (text !== '') {
        ...
        journalInput.value = ''; // Clear the input
    }
}

// Function to render the journal entries on the page
function renderEntries() {
    journalEntriesContainer.innerHTML = ''; // Clear previous content
    ...
    entries.forEach((entry, index) => {
        ...
        entryTime.innerText = entry.timestamp;
        ...
        updateButton.addEventListener('click', () => updateEntry(index));

        const deleteButton = document.createElement('button');
        deleteButton.innerText = 'Delete';
        deleteButton.addEventListener('click', () => deleteEntry(index));

        entryActions.appendChild(updateButton);
...
function updateEntry(index) {
    const newText = prompt('Update your journal entry:', entries[index].text);
    if (newText !== null && newText.trim() !== '') {
        entries[index].text = newText;
        entries[index].timestamp = new Date().toLocaleString(); // Update timestamp
        renderEntries();
    }
}
